...
void main() {
  group('ApiFutureEitherX', () {
    test('mapEntity should return Right with mapped data when response has data', () async {
      final future = Future<Either<AppException, BaseResponse<int>>>.value(
        Right(BaseResponse<int>(
          status: true,
          message: 'ok',
          data: 1,
        )),
  ...
  group('ApiFutureEitherMessageX', () {
    test('mapMessage should return Right(ResultMessage) with response message', () async {
      ...
      result.fold(
            (l) => fail('Expected Right but got Left'),
            (r) => expect(r.message, 'Success message'),
      );
    });
  });
  ...
  group('ApiFutureEitherPaginatedX', () {
    test('mapPaginated should map items correctly', () async {
      ...
      final future =
      Future<Either<AppException, BaseResponse<PaginatedResult<int>>>>.value(
        Right(BaseResponse<PaginatedResult<int>>(
          status: true,
          message: 'ok',
